...
void free_memory(sokoban_t* init_data, unsigned expanded_nodes ){
	for( unsigned i = 0; i < expanded_nodes; i++){
		freeNode(init_data, expanded_nodes_table[ i ]);
    }
...
void find_solution(sokoban_t* init_data, bool show_solution)
{
	...
	while(pq.count != 0){
		...
		for(i=left; i<=down; i++){
			node_t* newNode = NULL;
			bool playMoved = applyAction(init_data, n, &newNode, i);
			generated_nodes += 1;
			if(playMoved == false || simple_corner_deadlock(init_data, &(newNode -> state))) {
				freeNode(init_data, newNode);
				continue;
			}
			flatten_map(init_data, &flat_map, newNode->state.map);
			...
			heap_push(&pq, newNode);
		}
	}
	int j;
	for(j=0; j<pq.count; j++){
		freeNode(init_data, pq.heaparr[j]);
	}
	free(pq.heaparr);
...
void freeNode(sokoban_t* init_data, node_t* node){
	int i;
	for(i=0; i<init_data->lines; i+=1){
		free(node->state.map[i]);
	}
	free(node->state.map);
	free(node);
}
