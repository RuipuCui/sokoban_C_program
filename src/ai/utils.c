...
bool dead_square_deadlock(int x, int y, sokoban_t* init_data, state_t* state){
		if (((state->map[y][x+1] == '#' && state->map[y+1][x] == '#') ||
        (state->map[y+1][x] == '#' && state->map[y][x-1] == '#') ||
        (state->map[y][x-1] == '#' && state->map[y-1][x] == '#') ||
        (state->map[y-1][x] == '#' && state->map[y][x+1] == '#')) &&
		...
		int i;
		int goal_exit = 0;
		if((state->map[y][x+1] == '#' || state->map[y][x-1] == '#') && !is_goal_loc( state->player_y, state->player_x, init_data)){
			  for(i = 0; i < init_data->lines; i++){
					if(init_data->map_save[i][x] != '\0'){
						if(is_goal_loc(i, x, init_data)){
							  goal_exit = 1;
						}
					}
				}
				...
				goal_exit = 0;
		}else if((state->map[y+1][x] == '#' || state->map[y-1][x] == '#') && !is_goal_loc( state->player_y, state->player_x, init_data)){
			  for(i = 0; init_data->map_save[y][i] != '\0'; i++){
						if(is_goal_loc(y, i, init_data)){
							  goal_exit = 1;
						}
				}
				if(goal_exit != 1){
